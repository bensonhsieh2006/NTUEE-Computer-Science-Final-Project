...
\renewcommand{\ttdefault}{cmtt} % {cmtt}
\usepackage{listings}
\usepackage{upquote}
\usepackage{alltt}
\usepackage{framed}
...
\newcommand{\version}{3.0.2}        % !!! <<<=== Change here !!!
...
elif get_platform () == 'mingw_x86_64':  # Windows, MSYS2/ucrt64
    sdlbgilib = '/ucrt64/bin/SDL_bgi.dll'
...
the \p{x} and \p{y} coordinate of each vertex. The polygon is not
closed automatically.

In Python, \p{polypoints} is a list.
...
with information a\-bout the current fill pattern and fill colour.
...
Returns the size of the palette (\T{MAXCOLORS} + 1 in BGI mode, or
\T{MAXRGB\-COLORS} + 1 in ARGB mode).
...
\fn{lineto()}, \fn{rectan\-gle()}, \fn{drawpoly()}, etc. The line
style can be \T{SOLID\_LINE}, \T{DOTTED\_LINE}, \T{CEN\-TER\_\-LINE},
\T{DA\-SHED\-\_LINE}, or \T{USERBIT\_LINE}; in the latter case, the user
provides a 16-bit number (\p{u\-pat\-tern}) whose bits set to 1 will be
plotted as pixels.
...
vector fonts \T{TRI\-PLEX\_\-FO\-NT}, \T{SMALL\-\_FONT},
\T{SANS\_SERIF\_FONT}, \T{GOTHIC\_FONT}, \T{SCRIPT\_FONT},
\T{SIMPLEX\_FONT}, \T{TRI\-PL\-EX\_\-SCR\_FONT}), the text direction
...
will be scaled by the values set with \fn{setuser\-char\-si\-ze()}.
...
call to \fn{set\-text\-sty\-le()} set \p{charsize} to 0, the default
...
\T{XOR\-\_PUT}, \T{OR\_PUT}, \T{AND\_\-PUT}, and \T{NOT\_PUT}.
...
Closes the window identified by \p{id}. If \p{id} is \T{ALL\_WINDOWS},
\fn{closegraph()} is called.
...
\fn{getcolor()},\ \fn{putpixel()},\ \fn{set\-bk\-co\-lor()}
...
and \fn{set\-pa\-let\-te()} to set a colour as ARGB integer. The colour
...
\addcontentsline{toc}{subsection}{\T{clearmousebutton()}}
\phantomsection\label{sec:clearmousebutton}

\begin{bgi}
\Clogo{} \V{} \fn{clearmousebutton} (\I{} \p{kind});

\Plogo{} \fn{clearmousebutton} (\p{kind})
\end{bgi}

Clears mouse events; subsequent calls to \fn{ismouseclick()} will
return 0.
...
\T{SDL\_MOUSEBUTTONUP}, \T{SDL\_MOUSEWHEEL}, and \T{SDL\_\-QU\-IT}.
...
\T{SDL\_MOUSEBUTTONDOWN}, \T{SDL\_\-MOU\-SEWHEEL}, and \T{SDL\_QUIT}. It
...
buffer allocated with \fn{create\_string\_\-buf\-fer
...
environment variable \T{SDL\_\-BGI\-\_PALET\-TE} equals \T{BGI}, the
...
\T{SDL\-\_FULL\-SCREEN} will be used.  Unless a fullscreen window is
already present, multiple windows can be created.

The user must update the screen as needed using \fn{refresh()}, or
use \fn{sdlbgiauto()}.

This function is also overloaded as \fn{initwindow} (\I{} \p{width},
\I{} \p{height}, \C{}\p{title}) to specify the window title without
using \fn{setwinoptions()}.
...
(\fn{getmaxy() + 1}) $\times$ (\fn{get\-max\-x() + 1}) array of
...
\T{SDL\_WINDOWPOS\_CENTERED} or \T{SDL\_WINDOW\-POS\_UNDEFINED}.
...
\fn{getcolor()}, \fn{putpixel()}, \fn{set\-bk\-co\-lor()}
\fn{set\-bk\-color()}, \fn{setcolor()}, \fn{setfillpattern()},
\fn{setfill\-sty\-le()}, and \fn{setpa\-let\-te()} to set the colour from
the ARGB palette \p{color} entry. The colour index is
\T{ARGB\_\-TMP\_\-COL}.
...
be \T{SDL\_BLEND\-MO\-DE\-\_NONE} (default in ``slow mode'') or
...
\T{SDL\_WINDOWPOS\_CENTERED} or \T{SDL\_WINDOWPOS\_\-UN\-DE\-FI\-NED}.
...
values \T{SDL\_WINDOW\_\-FULL\-SCREEN},
\T{SDL\_WIN\-DOW\_FULLSCREEN\_DESKTOP}, \T{SDL\_WINDOW\_SHOWN},
\T{SDL\_WIN\-DOW\_\-HID\-DEN}, \T{SDL\_\-WINDOW\-\_BORDERLESS}, and
...
\p{left}, \p{top}---\p{right}, \p{bot\-tom}.
